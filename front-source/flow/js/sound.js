     ...
     methods: {
		...
		setActiveGroupType(type){
         ...
         removeSoundConfigGroup(){
             ...
             $.post(baseHost + '/services/sound/removeSoundConfigGroup.json?soundConfigGroupId='+data.soundConfigGroupId).then((res)=>{
                 if(res.resultMessageEnum=='0000'){
                     if(this.activeGroupType=='speech'){
         ...
         handleVideo(index,row){
        	 ...
        	 this.$nextTick(() => {
        		 ...
        		 this.viewSource = baseHost + '/services/file/downloadFile.html?filePath=' + row.soundPath;
             });
         },
         //得到文件播放时长
         getFilePlayDuration(){
             return $.get(baseHost + '/services/file/getFilePlayDuration.json?filePath=' + this.soundPath).then((res)=>{
                 if(res.resultMessageEnum=='0000'){
                     return res;
                 }else if(res.resultMessageEnum == "0001"){
                	 soundVue.$message.error(winOpener.$.utils.getErrorMessage(res.errorInfoList));
